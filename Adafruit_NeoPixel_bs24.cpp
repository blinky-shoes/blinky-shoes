...
Adafruit_NeoPixel_bs24::Adafruit_NeoPixel_bs24(uint16_t n, uint8_t p, uint8_t t) :
...
Adafruit_NeoPixel_bs24::~Adafruit_NeoPixel_bs24() {
  if(pixels) free(pixels);
  pinMode(pin, INPUT);
}

void Adafruit_NeoPixel_bs24::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void Adafruit_NeoPixel_bs24::show(void) {
...
void Adafruit_NeoPixel_bs24::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void Adafruit_NeoPixel_bs24::setPixelColor(
...
void Adafruit_NeoPixel_bs24::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t Adafruit_NeoPixel_bs24::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t Adafruit_NeoPixel_bs24::getPixelColor(uint16_t n) const {
  if(n >= numLEDs) {
...
uint8_t *Adafruit_NeoPixel_bs24::getPixels(void) const {
  return pixels;
}

uint16_t Adafruit_NeoPixel_bs24::numPixels(void) const {
  return numLEDs;
}
...
void Adafruit_NeoPixel_bs24::setBrightness(uint8_t b) {
...
uint8_t Adafruit_NeoPixel_bs24::getBrightness(void) const {
  return brightness - 1;
}

void Adafruit_NeoPixel_bs24::clear() {
  memset(pixels, 0, numBytes);
}
